بطور کلی Pipe ها کلاس هایی هستند که ایتفرفیس `PipeTransform` رو پیاده سازی کردند و بالاشون از دکوراتور `@Injectable` استفاده شده است.
...
>[!tip]
 >اگر pipe ها exception تولید کنند ، توسط سیستم مدریت خطای نست (Global Exception filter) این خطای پرتاب شده از Pipe ها مدیریت می شود.
...
اکنون می خواهیم مثالی از `ParseIntPipe` بزنیم ، لازم به ذکره که بقیه pipe ها که با Parse شروع می شوند همگی پیاده سازی شون یکسان هست.
## Binding Pipes--------------
...
می تونیم pipe های اختصاصی خودمون رو توسعه بدیم ، در مثال پایین یک لوله validation ساختیم که در آن یک مقدار رو گرفتیم و همونو return کردیم :
...
همه pipe ها باید اینترفیس PipeTransform رو پیاد سازی کنن ، در این اینترفیس متد transform() وجود داره که باید پیاده سازی بشه 

این متد یعنی متد transform() دو پارامتر ورودی میگیره:
- پارمتر اول `value`
- پارامتر دوم `metadata`

پارامتر اول یعنی Value همان آرگومان ورودی controller مربوطه هست و پارامتر دوم یعنی metadata شامل اطلاعات پردازش فعلی (request , response , ...) هست.

یه object از اینترفیس metadata شامل property های زیر هست :
...
می خواهیم مطمئن شویم که body دریافت شده از request معتبر است ؛ حال body از نوع CreateCatDto ، بنابراین باید هر یک از property های آن را از نظر معتبربودن یا نبودن بررسی کنیم.

ما می تونیم این کار رو در routeHandler مربوطه یعنی در متد create انجام دهیم ، اما انجام این کار اصل تک مسئولیتی (single responsibility principle) رو نقض میکنه.

راهکار دیگه ایجاد یک کلاس اعتبارسنجی (validator class) و واگذاری وظیفه اعتبارسنجی به اون کلاسه ؛ که این روش هم مشکلاتی داره از جمله اینکه باید همیشه یادمون باشه که این کلاس اعتبارسنجی رو بالای هر متدی صدا بزنیم.

چطوره از یک middleware عمومی ها استفاده کنیم ؟ middleware ها به دلیل اینکه از controller ای که می خواد بعدش اجرا بشه و همچنیم پارامتر های ورودی این controller اطلاعی ندارن ، راهکار خوبی نیستند.
...
حالا کلاس ValidationPipe ای می سازیم که از این class-validator استفاده می کند:
...
درباره دو بخش بالا بعدا توضیح داده خواهد شد....
...
با اینکار فارغ از اینکه این pipe در کدام مازول تعریف شده ، در سراسر پروژه مورد استفاده قرار میگیرد و روی هر کنترل و route handler ای اعمال می شود.
...
حالا برای اینکه به endpoint ها بتونیم این امکان رو بدیم که داده از دست رفته رو handle کنند یه pipe ای به اسم defaultValuePipe رو قبل از parsePipe ها قرار می دیم.
